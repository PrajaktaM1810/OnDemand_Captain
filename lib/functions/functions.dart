...
getvehicleType() async {
  myServiceId = isowner == true ? userDetails['service_location_id'] : myServiceId;

  // Print the API endpoint and parameters for debugging
  print('Debug: API Endpoint - ${url}api/v1/types/$myServiceId?transport_type=$transportType');
  print('Debug: myServiceId - $myServiceId');
  print('Debug: transportType - $transportType');
...
Map<String, dynamic> subscriptionListPage = {};

Future<Map<String, dynamic>> getSubscriptionList(int driverId) async {
  try {
    var response = await http.post(
      Uri.parse('${url}api/v1/request/list-Subscription'),
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer ${bearerToken[0].token}'
      },
...
Future<String> subscribePackage(int driverId, int packageId, String trId, String amount) async {
  dynamic result;
  try {
    var headers = {
      'Content-Type': 'application/json',
...
Future<String> getQrCodeImg() async {
  try {
    var response = await http.get(
      Uri.parse('${url}api/v1/request/getqrcode'),
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer ${bearerToken[0].token}'
      },
    ...
    if (response.statusCode == 200) {
      ...
      if (responseData['success'] == true && responseData['data'] != null) {
        return '$url/public/storage/uploads/system-admin/logo/${responseData['data']}';
      }
    } else if (response.statusCode == 401) {
      return 'logout';
    }
...
List<Map<String, dynamic>> myTargets = [];
Map<String, dynamic> myTargetsPage = {};

Future<String> getMonthlyTargets() async {
  String result;

  try {
    var response = await http.get(
      Uri.parse('https://admin.nxtdig.in/api/v1/request/listAllMonthlyTargets'),
      headers: {
        'Authorization': 'Bearer ${bearerToken[0].token}',
        'Content-Type': 'application/json'
      },
    ...
    print('Headers: Bearer ${bearerToken[0].token}');

    if (response.statusCode == 200) {
      var decodedResponse = jsonDecode(response.body);

      if (decodedResponse['success'] == true) {
        myTargets = List<Map<String, dynamic>>.from(decodedResponse['data']);
        myTargetsPage = decodedResponse.containsKey('meta') ? decodedResponse['meta'] : {};

        // Remove empty elements safely
        myTargets.removeWhere((element) => element.isEmpty);
...
Future<Map<String, dynamic>> getRewardHistory(int driverId) async {
  const String url = 'https://admin.nxtdig.in/api/v1/request/rewards_history';
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      if (decodedResponse['success'] == true && decodedResponse['data'] is List) {
        ...
        return {'status': 'no_data', 'data': [], 'message': decodedResponse['message'] ?? 'No data available'};
      }
    } else if (response.statusCode == 401) {
      return {'status': 'logout', 'data': [], 'message': 'Unauthorized access. Please log in again.'};
    } else {
      return {'status': 'failure', 'data': [], 'message': 'Something went wrong. Please try again later.'};
    }
  } catch (e, stackTrace) {
    print('Error fetching reward history: $e');
    print(stackTrace);
    return {'status': 'failure', 'data': [], 'message': 'An error occurred while fetching reward history.'};
  }
}

Future<Map<String, dynamic>> getSubscriptionHistory(int driverId) async {
  const String url = 'https://admin.nxtdig.in/api/v1/request/Subscription-history';
  ...
  try {
    ...
    print(stackTrace);
    return {'status': 'failure', 'data': [], 'message': 'An error occurred while fetching subscription history.'};
  }
}

//  SubscriptionStatus

Future<Map<String, dynamic>> subscriptionStatus(int driverId) async {
  try {
    const url = 'https://admin.nxtdig.in/api/v1/request/driver-active-subscription';
    ...
    if (response.statusCode == 200) {
      ...
      if (json['success'] == true) {
        return {
          'status': json['data'] is List ? 'success' : 'no_data',
...
// DRIVER RENT FUNCTIONALITY

  Future<Map<String, dynamic>> getRequestList() async {
    const String url = 'https://admin.nxtdig.in/api/v1/request/getAvailableRequestsForDriver';
    try {
      final response = await http.get(
        Uri.parse(url),
        headers: {
  ...
  Future<Map<String, dynamic>> updateDriverRequestStatus({
    required int requestId,
    required String status,
    required int expectedPrice,
  }) async {
    const String url = 'https://admin.nxtdig.in/api/v1/request/updateDriverRequestStatus';
